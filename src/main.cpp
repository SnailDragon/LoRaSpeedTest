...
void loop() {
  ...
  if(millis() - last_calc > 1000){
    ...
    if(bits_received != 0){
      ...
      average_bps = (average_bps * reading_count + packet_bps) / (reading_count+1);
      reading_count++; 
      bits_received = 0; 
    }
  }
  ...
  display.drawString(0,10, "It: " + String(it) + " R: " + String(received_count));
  display.drawString(0,22, "Rate: " + String(packet_bps) + "bps (B: " + String(best_bps) + ")"); 
